...
#Editted the query, because we want to set/check the ttl based on feed.name; because our classification.type and classification.identifier are not well defined
#TODO, change the remark above
SELECT_QUERY = '''
...
class SquelcherExpertBot(Bot):
    ...
    def process(self):
        ...
        try:
            self.cur.execute(SELECT_QUERY, (ttl, event['feed.name'],
                                            event['source.ip']))
